...
// Translator converts canonical OpenAI request/response shapes to a specific backend provider's native API and vice versa.
...
// ErrUnsupportedEndpoint is returned when a translator does not implement a given endpoint (e.g., Anthropic has no rerank API).
...
// NewRegistry returns the default registry populated with all built-in translators (llamacpp, openai, anthropic, google).
func NewRegistry() *Registry {
	r := &Registry{byName: make(map[string]Translator, 4)}
